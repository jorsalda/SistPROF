...
def gestion_carga_academica():
    """Vista global para asignar MATERIAS a grupos con docentes"""
    from app.models.grupo_materia import GrupoMateria
    from app.models.plan_estudios import PlanEstudios
    ...
    if grado_url:
        grupos_query = grupos_query.filter_by(grado=grado_url)

    grupos = grupos_query.order_by(Grupo.grado, Grupo.nombre).all()

    # 2. LÓGICA DE FILTRADO ROBUSTA Y HORAS POR GRADO
    materias = []
    horas_plan_total = 0
    horas_plan_dict = {}

    if grupos:
        # A. Obtener materias basadas en el primer grupo (para mostrar columnas)
        grado_raw_ref = grupos[0].grado
        grado_limpio_ref = grado_raw_ref.replace('°', '').replace('º', '').strip()

        plan_items_ref = PlanEstudios.query.filter(
            PlanEstudios.colegio_id == current_user.colegio_id,
            func.replace(func.replace(PlanEstudios.grado, '°', ''), 'º', '') == grado_limpio_ref,
            PlanEstudios.activo == True,
            PlanEstudios.horas_semanales > 0
        ).all()

        materia_ids = [p.materia_id for p in plan_items_ref]

        if materia_ids:
            materias = Materia.query.filter(Materia.id.in_(materia_ids)).order_by(Materia.nombre).all()

            # Calcular total de horas para el badge (basado en el grado de referencia)
            horas_plan_total = sum(int(p.horas_semanales) for p in plan_items_ref)

        # B. CORRECCIÓN CRÍTICA: Llenar horas_plan_dict para TODOS los grados de los grupos mostrados
        # Esto asegura que si hay grupos de 10° y 11°, ambos tengan sus horas correctas
        grados_unicos_en_lista = list(set([g.grado for g in grupos]))

        for grado_grupo in grados_unicos_en_lista:
            grado_limpio_grupo = grado_grupo.replace('°', '').replace('º', '').strip()

            # Buscar plan para este grado específico
            plan_items_grupo = PlanEstudios.query.filter(
                PlanEstudios.colegio_id == current_user.colegio_id,
                func.replace(func.replace(PlanEstudios.grado, '°', ''), 'º', '') == grado_limpio_grupo,
                PlanEstudios.activo == True,
                PlanEstudios.horas_semanales > 0
            ).all()

            # Guardar en diccionario usando el grado ORIGINAL del grupo (para que coincida en template)
            for p in plan_items_grupo:
                horas_plan_dict[(grado_grupo, p.materia_id)] = int(p.horas_semanales)

    # FALLBACK CONTROLADO
    if not materias and grupos:
        flash("⚠️ No se encontró configuración de Plan de Estudios para este grado. Mostrando todas las materias.",
              "warning")
    ...
    return render_template(
        ...
        horas_plan_total=horas_plan_total,
...
def guardar_carga_academica():
    """Guarda asignaciones con validaciones"""
    from app.models.grupo_materia import GrupoMateria
    from app.models.plan_estudios import PlanEstudios
    ...
    try:
        ...
        for grupo in grupos:
            for materia in materias:
                ...
                if doc_id and horas > 0:
                    nuevas_asignaciones[(grupo.id, materia.id)] = {'docente_id': doc_id, 'horas': horas, 'grupo': grupo,
                                                                   'materia': materia}

        # Validar horas por grado vs Plan
        horas_por_grado = {}
        ...
        errores_grado = {}
        for grado, h_asignadas in horas_por_grado.items():
            h_plan = sum(p.horas_semanales for p in
                         PlanEstudios.query.filter_by(colegio_id=current_user.colegio_id, grado=grado,
                                                      activo=True).all())
            if h_asignadas > h_plan:
                errores_grado[grado] = {'plan': h_plan, 'asignadas': h_asignadas}
        ...
        # Validar máximo 22h por docente
        horas_por_docente = {}
        for (g_id, m_id), data in nuevas_asignaciones.items():
        ...
        for grupo in grupos:
            for materia in materias:
                doc_id = request.form.get(f'docente_{grupo.id}_{materia.id}', type=int)
                horas = request.form.get(f'horas_{grupo.id}_{materia.id}', 0, type=int)
                asignacion = GrupoMateria.query.filter_by(grupo_id=grupo.id, materia_id=materia.id, activo=True).first()
                ...
                if doc_id and horas > 0:
                    if asignacion:
                        ...
                        db.session.add(GrupoMateria(grupo_id=grupo.id, materia_id=materia.id, docente_id=doc_id,
                                                    horas_semanales=horas, activo=True))
                        cambios += 1
                elif asignacion:

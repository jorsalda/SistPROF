...
def nuevo_docente():
    ...
    if request.method == "POST":
        ...
        if not sede_id:
            ...
            return redirect(url_for("colegio.nuevo_docente"))
        existe = Docente.query.filter_by(nombre=nombre, colegio_id=current_user.colegio_id).first()
        if existe:
...
# ==========================================================
# RUTAS FALTANTES (Para que el menú del Colegio funcione sin errores)
# ==========================================================
@colegio_bp.route("/estudiantes")
@login_required
def lista_estudiantes():
    flash("El módulo de gestión de Estudiantes para el Colegio está en construcción.", "info")
    return redirect(url_for('colegio.dashboard'))

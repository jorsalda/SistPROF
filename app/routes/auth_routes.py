...
def index():
    ...
    if current_user.is_authenticated:
        ...
        # ADMIN COLEGIO
        elif current_user.rol == 'admin_colegio':
        ...
        # ACUDIENTE
            ...
            return redirect(url_for('acudiente.dashboard'))
...
def login():
    ...
    if request.method == 'POST':
        ...
        if ok:
            ...
            # ADMIN COLEGIO
            elif resultado.rol == 'admin_colegio':
                return redirect(url_for('colegio.dashboard'))

            # COORDINADOR
            elif resultado.rol == 'coordinador':
                return redirect(url_for('coordinador.dashboard'))

            # DOCENTE
            elif resultado.rol == 'docente':
                return redirect(url_for('docente.dashboard'))
...
def reset_password(token):
    ...
    if not email:
        ...
        flash(
            "❌ Token inválido o expirado (tiempo límite: 1 hora)",
            "danger"
        )
    ...
    if request.method == 'POST':
        ...
        ok, mensaje = resetear_contrasena_por_email(
            email,
            nueva_contrasena
        )
...
# ⭐⭐ MANEJADOR RATE LIMIT ⭐⭐
@auth_bp.errorhandler(RateLimitExceeded)
def handle_rate_limit(error):

    flash(
        "⚠️ Demasiados intentos. Espera un minuto e intenta de nuevo.",
        "danger"
    )
